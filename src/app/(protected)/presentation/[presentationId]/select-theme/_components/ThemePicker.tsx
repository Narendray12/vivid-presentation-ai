...
const ThemePicker = ({ selectedTheme, themes, onThemeSelect }: Props) => {
  ...
  const { projects, setSlides, currentTheme } = useSlideStore();
  ...
  const handleGenerateLayouts = async () => {
    ...
    // Check if projects array exists and has at least one project
    ...
    const currentProject = projects[0]; // Get the first project from the array
    ...
    try {
      ...
      router.push(`/presentation/${currentProject.id}`);
      setSlides(res.data);
    } catch (error) {
