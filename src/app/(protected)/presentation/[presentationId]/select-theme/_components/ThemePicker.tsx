...
const ThemePicker = ({ selectedTheme, themes, onThemeSelect }: Props) => {
  ...
  const handleGenerateLayouts = async () => {
    ...
    try {
      ...
      router.push(`/presentation/${projects?.id}`);
      setSlides(res.data);
    } catch (error) {
      toast.error("Error", {
        description: "Error generating layouts",
  ...
  return (
    <div
      className="w-[400px] overflow-hidden stricky top-0 h-screen flex flex-col"
      style={{
        backgroundColor:
          selectedTheme.sidebarColor || selectedTheme.backgroundColor,
        borderLeft: `1ps solid ${selectedTheme.accentColor}20`,
      }}
    >
